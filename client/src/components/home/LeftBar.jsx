...
const Leftbar = () => {
  const links = [
    { href: '/account-settings', label: 'Technology' },
  ...
  return (
    <div className="relative">
      ...
      <Menu.Items className='flex flex-col absolute top-10  -left-12 shadow-xl px-9 py-6 rounded-xl text-md  font-medium bg-white'>
        {links.map((link) => (
          <Menu.Item
            as="a"
            key={link.href}
            href={link.href}
            className="flex flex-col gap-2 hover:bg-primary rounded-full px-3 py-2 text-center"
          >
            {link.label}
          </Menu.Item>
        ))}
