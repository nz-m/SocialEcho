...
router.post("/", async (req, res) => {
  ...
  try {
    const savedUserContext = await newUserContext.save();
    res
      .status(200)
      .json({ message: "Context data saved successfully!", savedUserContext });
  } catch (error) {
...
router.get("/", async (req, res) => {
  try {
    const userContextData = await UserContext.find();
    res
      .status(200)
      .json({ message: "Context data fetched successfully!", userContextData });
  } catch (error) {
    console.log(error);
  }
});
